...
const (
	...
	VALIDATION_ERROR_EXTENSION = "The file extension is not allowed. Allowed extensions are: %v"
...
func (v Validator) New(ctx *fasthttp.RequestCtx) Validator {
	newValidator := Validator{ctx, nil, nil, "", make(map[string]interface{})}
	newValidator.Fields = make(map[string]map[string]interface{});
	newValidator.Errors = make(map[string]error);
	return newValidator;
}
...
func (v *Validator) ValidateRequired(key string, option interface{}, isMultiValue bool) (bool, error) {
	...
	if (v.isEmpty(key)) {
		return false, errors.New(VALIDATION_ERROR_REQUIRED);
	}
...
func (v *Validator) validateUnique(key string, option interface{}, required bool) (bool, error) {
	...
	if (countMax < count) {
		return false, errors.New("The database already contains an entry with the same value.")
	}
...
func (v *Validator) ValidateExtension(key string, option interface{}, required bool) (bool, error) {
	...
	return false, errors.New(fmt.Sprintf(VALIDATION_ERROR_EXTENSION, strings.Join(allowedExts, ",")));
}
...
func (v *Validator) ValidateFormat(key string, option interface{}, required bool) (bool, error) {
	...
	case "email":
		...
		err = errors.New(VALIDATION_ERROR_EMAIL);
		break;
	case "link":
		valid = v.validateLink(postVal);
		err = errors.New(VALIDATION_ERROR_LINK);
		break;
	case "regexp":
		valid = v.validateRegexp(postVal, optionMap["regexp"].(string));
		err = errors.New(VALIDATION_ERROR_REGEXP);
		break;
	case "password":
		valid = v.validatePassword(postVal);
		err = errors.New(VALIDATION_ERROR_PASSWORD);
		break;
	}
...
func (v *Validator) validateRegexp(value string, exp string) (bool) {
	h.PrintlnIf(fmt.Sprintf("Validating regexp %v in %v", exp, value), h.GetConfig().Mode.Debug)
...
func (v *Validator) ValidateLength(key string, option interface{}, required bool) (bool, error) {
	...
	if ((okMin && len(postVal) < minLength) || (okMax && len(postVal) > maxLength)) {
		return false, errors.New(VALIDATION_ERROR_LENGTH);
	}
	return true, nil;
}
...
func (v *Validator) ValidateSameAs(key string, option interface{}, required bool) (bool, error) {
	...
	if (postVal != sameValue) {
		return false, errors.New(VALIDATION_ERROR_SAMEAS);
	}
