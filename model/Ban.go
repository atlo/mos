...
func (b Ban) IsBanned(RemoteAddress string) bool {
	var ban Ban;
	var query string = fmt.Sprintf("SELECT * FROM %s WHERE `remote_address` = '%s' AND `until` >= '%s' ORDER BY `until` DESC LIMIT 1",b.GetTable(), RemoteAddress, h.GetTimeNow().Round(time.Second).Format(MYSQL_TIME_FORMAT));
