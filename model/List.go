...
func (l *List) GetOrder() string {
	...
	if order == "" {
		order = strings.Join(l.PrimaryKey, ", ");
	}
	return order
}

func (l *List) GetOrderDir() string {
	orderdir := string(l.Ctx.QueryArgs().Peek(l.OrderDirParam))
	if orderdir == "" {
		orderdir = "DESC"
	}
	return orderdir
...
func (l *List) GetCount() int64 {
	...
	query := fmt.Sprintf("SELECT COUNT(m.`%v`) FROM %v%v",l.PrimaryKey[0], l.GetTablesSql(), where)
