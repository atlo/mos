...
func (c Config) GetAll() []Config {
	var Configs []Config
	_, err := db.DbMap.Select(&Configs, fmt.Sprintf("select * from %s order by %v", c.GetTable(), c.GetPrimaryKey()[0]));
...
func (_ Config) Get(ConfigId int64) (Config, error) {
	...
	err := db.DbMap.SelectOne(&Config, fmt.Sprintf("SELECT * FROM %s WHERE %v = ?", Config.GetTable(), Config.GetPrimaryKey()[0]), ConfigId);
	h.Error(err, "", h.ERROR_LVL_ERROR);
	if (err != nil) {
		return Config, err;
	}
...
func (c Config) GetByPath(path string, languageCode string) (Config, error) {
	var Config Config
	var query string = fmt.Sprintf("SELECT * FROM %s WHERE %v = ?", c.GetTable(), "path")
...
func (c Config) GetValueByPath(path string) string {
	var query string = fmt.Sprintf("SELECT `value` FROM %s WHERE %v = ?", c.GetTable(), "path")
	h.PrintlnIf(query, h.GetConfig().Mode.Debug);
